...
@router.post(
    ...
    description="Start a new data collection job for brand comparison analysis"
)
async def start_data_collection(request: CollectionRequest):
    ...
    try:
        ...
        invalid_sources = [source for source in request.sources if source not in settings.available_sources]
        if invalid_sources:
        ...
        job_id = await job_manager.start_collection_job(request)

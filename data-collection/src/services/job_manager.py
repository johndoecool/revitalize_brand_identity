    ...
    async def _collect_brand_data(
        ...
        try:
            ...
            async def progress_callback(message: str, completed_source: DataSource = None):
                ...
                if current_job:
                    # Update individual source completion
                    if completed_source and completed_source not in current_job.completed_sources:
                        current_job.completed_sources.append(completed_source)
                        # Remove from remaining sources (avoid duplicates)
                        ...
                        logger.info(f"Source {completed_source.value} completed for job {job_id}")
                    
                    # Calculate progress based on completed sources
                    completed_count = len(current_job.completed_sources)
                    total_sources = len(current_job.sources) * 2  # For both brand and competitor
